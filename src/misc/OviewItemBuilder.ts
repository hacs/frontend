...
export class OviewItemBuilder {
    ...
    constructor(configuration: Configuration, lovelaceconfig: LovelaceConfig, status: Status, route: Route) {
    ...
    render_list_line(repository: Repository) {
        return html`
        <paper-item .RepoID=${repository.id} @click="${this.ShowRepository}"
          class="${(this.configuration.frontend_compact ? "compact" : "")}">
          <div class="icon">
            <ha-icon
              icon="mdi:new-box"
    ...
    render(repository: Repository) {
        if (this.configuration.frontend_mode === "Grid") return this.render_card(repository)
        return this.render_list_line(repository)
    }

    ShowRepository(ev: { composedPath: () => { forEach: (arg0: (item: any) => void) => void; }; }) {
        var RepoID: string
